...
Motor motorSalt = Motor(1, 1, false, NO_STOP_PIN, 512);  //Motor(int registerNr, int firstOrSecondBlock, bool isPin2and3Reversed, int stopPin) //first registerNr is 1, first block(0,1,2,3) is 1, second block(4,5,6,7) is 2
Motor motorSugar = Motor(1, 2, false, NO_STOP_PIN, 512);  
Motor motorWeijing = Motor(2, 1, false, NO_STOP_PIN, 512);    

Motor motorTurnTotal = Motor(1, 2, false, turnPinTotal, 512); 
Motor motorMoveTotal = Motor(2, 2, false, stopPinTotal, 512); 
Motor motorMoveOil = Motor(1, 1, false, stopPinMoveOil, 512); 
Motor motorMoveTemperatorSensor = Motor(2, 1, false, stopPinMoveTemperatorSensor, 512); //not used 
...
{  
    ...
    motorGroup2.driveMotors(motorTurnTotal, 512);                     
