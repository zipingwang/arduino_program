...
{         
  ...
  if (readline(bluetooth.read(), buffer, BUFFER_SIZE) > 0) {
    ...
    {
      unsigned char commandc[BUFFER_SIZE]={'A',1,'A',2,'A',13,'F',1,10,'A',4,'ÿ'}; //must use BUFFER_SIZE
      commandc[8]= buffer[1];
      carryOutCommands(commandc);
    }
    else if (buffer[0]==(byte)'d')
    {
      unsigned char commandd[BUFFER_SIZE]={'A',1,'A',2,'A',13,'O',1,90,'F',1,0,'A',4,'ÿ'};
      commandd[8]=buffer[1];
      commandd[11]=buffer[2];
