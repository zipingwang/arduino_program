  ...
  {		
    ...
    case 'B':			    
      ...
      else if(nextByte==100)
      {
        ...
        int steps=value*STEPS_PER_TURN/UNIT_PER_TURN;
        doTakeSalt(steps);				
      }
      ...
      else if(nextByte==103)
      {
        ...
        {						
          ...
          int steps=value*STEPS_PER_TURN/UNIT_PER_TURN;
          //nagative is anticlockwise, if face axis.
          steps=-steps;
          if(nextByte==100)
          {	            
    ...
    case 'C':
      ...
      {
        //  11. adjust cooking time
        if(_adjustTimeDone==false)
        {
          currentPage=PAGE_ADJUST_TIME;
          currentIndex=PAGE_ADJUST_TIME_SIZE/2;
          displayLcd(currentPage, currentIndex); 
          
          memcpy(_commands,commands, BUFFER_SIZE);
          return; //return and will carry the command again
        }
        else
        {
            _requestHandled=false;   
      
           displayAdjustTimeValueLcd();
          ...
          /*
          _requestAdjustTime=true;
          while(!_adjustTimeDone)
          {
            delay(100);
          }  
         _requestAdjustTime=false; 
    ...
    case 'F':
      ...
      if(nextByte==2 || nextByte==5)
        doStirMotorOn();
        
      if(nextByte==4 || nextByte==5 || nextByte==6)
      {
        if(abs(_adjustTime)>value && _adjustTime <0)
        {
          _adjustTime=_adjustTime + value;
          value=0;
        }
        else
        {            
          value=value+_adjustTime;
          _adjustTime=0;
        }
      }
        
      if(nextByte==1 || nextByte==2 || nextByte==4 || nextByte==5)
      {
        //1. cooking x sconds
        //2. cooking x sconds with continue stiring 
        //4. cooking x sconds adjustable with C11
        //5. cooking x sconds with continue stiring adjustable  with C11
      ...
      else if (nextByte==3 || nextByte==6)
      {
        //3. cooking x sconds with stiring(pause)
        //6. cooking x sconds with stiring(pause) adjustable  with C11
...
void moveTotal()
{
  ...
  motorGroup2.driveMotors(motorTurnTotal, 500); 
  delay(1000);
  ...
  motorGroup2.driveMotors(motorMoveTotal, 180);
  delay(1000);

  //turn
  motorTurnTotal.stopPin=NO_STOP_PIN;
  motorGroup2.driveMotors(motorTurnTotal, -330);
  delay(1000);
...
void displayAdjustTimeValueLcd()
{
   LCDA.CLEAR();
   char buffer[5];
   itoa(_adjustTime, buffer, 10);
   displayLineInLcd(0, buffer);    
   currentIndex=0;  
}
...
void doSelfTest()
{
  ...
  connectRelayPin(22, 1000); //22 is not connected, but should hear click of relay
  connectRelayPin(pinNengtouHeatBottom, 1000);  //24
  connectRelayPin(pinNengtouMotor, 1000);  
  connectRelayPin(pinNengtouHeatTop, 1000);  //28
  connectRelayPin(pinOil, 1000);
  connectRelayPin(pinWater, 1000);  
  connectRelayPin(pinLaoChou, 1000);  
  connectRelayPin(pinQianFeng, 1000);  
}
/*
