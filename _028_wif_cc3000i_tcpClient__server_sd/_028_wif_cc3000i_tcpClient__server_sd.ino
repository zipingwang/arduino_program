//
//must change standaart adafruit file: Adafruit_CC3000.cpp(already done)
...
//run this sktech, note myip. after several seconds, should see "I an arduino cc3000" on simpleServer side.
//run simpleTcpClient, fill myip and listen port 23.
//if run on arduino uno, comment the testSD code, because the sketch is too big for uno
...
uint32_t serverIp= ((uint32_t)192<<24) | ((uint32_t)168<<16) | ((uint32_t)1<<8) | 7; // 3232235784; //192.168.1.8;
//ip=192;
//ip=ip<<24

//Adafruit_CC3000_Client www; //= cc3000.connectTCP(serverIp, 9100);
Adafruit_CC3000_Server chatServer(23);
...
void setup(void)
{
  ...
  */
  ...
  //www = cc3000.connectTCP(serverIp, 9100);
...
void testSD()
{
  ...
  if (myFile) {
    ...
    if (www.connected()){
      www.fastrprint("DISCONNECT"); //let simpleTCP to close socket on simpleTCP side
      www.close();
    }
    // close the file:
    myFile.close();
